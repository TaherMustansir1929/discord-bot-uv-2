...
class GoogleImagenImageGenerator:
    def __init__(self, api_key: str, base_url: str = "https://api.a4f.co/v1"):
        ...
        log_debug(
            f"Initialized GoogleImagenImageGenerator with base URL: {self.base_url}"
        )

    def generate_image(
        self,
        prompt: str,
        model: str = "google-imagen",
        width: int = 1024,
        height: int = 1024,
        steps: int = 25,
        ...
        Args:
            ...
            model: Model to use (google-imagen, provider-4/imagen-4, etc.)
        ...
        log_debug(
            f"Dimensions: {width}x{height}, Steps: {steps}, Guidance: {guidance_scale}"
        )
        ...
        try:
            log_info("Sending request to Google Imagen API...")
            ...
            log_success("Successfully received response from Google Imagen API")
            ...
            error_msg = f"Google Imagen API request failed: {str(e)}"
    ...
    def save_image_from_response(
        ...
        output_dir: Union[str, Path] = "images/generated_images",
    ) -> str:
        """
        ...
        try:
            ...
            if filename is None:
                ...
                filename = f"google_imagen_image_{timestamp}.png"
            ...
            if "b64_json" in image_data:
                ...
                error_msg = (
                    "No valid image data found in response (missing b64_json or url)"
                )
                log_error(error_msg)
    ...
    def generate_and_save(
        self, prompt: str, filename: Optional[str] = None, **kwargs
    ) -> Tuple[str, str]:
        """
        ...
        try:
            ...
            log_success(
                f"Image generation and save completed in {total_time:.2f} seconds"
            )
...
def google_imagen_image_generator(prompt: str) -> Tuple[Optional[str], Optional[str]]:
    """
    ...
    try:
        generator = GoogleImagenImageGenerator(api_key)
        log_debug("Initialized GoogleImagenImageGenerator")
...
if __name__ == "__main__":
    google_imagen_image_generator(
        "A majestic dragon soaring through cloudy skies at sunset, photorealistic, highly detailed"
    )
